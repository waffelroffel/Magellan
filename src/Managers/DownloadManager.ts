...
import { ActionTypes } from "../IndexTable";
...
export default class DownloadManager {
	...
	add(socket: Socket): void {
		socket.on("data", (data) => {
			...
			const json: Payload = JSON.parse(this.IOmap.get(socket)?.toString() || "{}")
			if (!json.name) return
			// index
			if (!this.Magellan.index.apply(json.name, json.meta)) return
			// io
			if (json.data && (json.meta.lastAction === ActionTypes.Change || json.meta.lastAction === ActionTypes.Add))
				writeFileSync(this.Magellan.root + "\\" + json.name, Buffer.from(json.data))
